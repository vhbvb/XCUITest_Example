...
//                self._typeText(text, textField:textField)
    ...
    func testAutoClick()
    {
        ...
        for _ in 0...100
        {
            ...
            for _ in 0...2*allHittableElements.count
            {
                sleep(1)
                let elem = allHittableElements[Int(arc4random_uniform(UInt32(allHittableElements.count)))]
                
                if elem.exists && elem.isHittable {
                    elem.tap()
                    print("----------------------------> Element:\(elem.description) tapped")
                }
                else
                {
                    break;
